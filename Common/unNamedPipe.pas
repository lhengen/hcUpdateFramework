...
type
  ...
  ////////////////////////////////////////////////////////////////////////////////
function ExecConsoleEvent(ProcessHandle: NativeInt; Event: DWORD): Boolean;
procedure ExitProcessEx(ProcessHandle: NativeInt; ExitCode: DWORD);
function GetConsoleWindowEx(ProcessHandle: NativeInt; ProcessID, ThreadID: DWORD): HWND;
...
function GetConsoleWindow(ProcessHandle: NativeInt): HWND;
var
...
begin
  ...
  begin
    // Write memory
    WriteProcessMemory(ProcessHandle, lpConsoleHwnd, @ConsoleWindow, dwSize, dwWrite);
    // Resource protection
    try
    ...
    CTRL_C_EVENT:
      begin
        ...
        if Assigned(lpCtrlEvent) then
          WriteProcessMemory(ProcessHandle, lpCtrlEvent, @CtrlC, dwSize, dwWrite);
      end;
    // Control break
    CTRL_BREAK_EVENT:
      begin
        ...
        // Check memory, write code from this process
        if Assigned(lpCtrlEvent) then
          WriteProcessMemory(ProcessHandle,lpCtrlEvent, @CtrlBreak, dwSize, dwWrite);
      end;
  else
    // Not going to handle
    lpCtrlEvent := nil;
...
begin
  ...
  begin
    // Enter critical section
    if InstCritSect.DebugInfo <> nil then
      EnterCriticalSection(InstCritSect);
    // Resource protection
    try
      // Get instance block
